...
pub fn AuthBackground() -> impl IntoView {
    ...
    loop {
        ...
        let instruction = Z80_PARSER
            .ins_from_machinecode(&emulator.memory, current_address as u16)
            .expect("Failed to parse instruction");
        instructions.insert(
            current_address,
            (instruction.to_string(), current_instruction),
        );
    ...
    let active = move || {
        instructions.with(|instructions| {
            instructions
                .get(&emu_signal.with(|emu| emu.cpu.registers.pc))
